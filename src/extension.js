...
function activate(context) {
    ...
    // Функция для обновления декораций (подсчет слогов)
    ...
    function generatePoetMap(line) {
        const cleanedLine = line.replace(/[^а-яa-zё]/gi, ''); // Убираем спецсимволы, знаки препинания и пробелы
        return cleanedLine
            .replace(/([аеёиоуыэюяaeiouy])/gi, '|') // Гласные -> '|'
            .replace(/([^аеёиоуыэюяaeiouy]+)/gi, '_'); // Несколько согласных -> '_'
    }
    // Функция для обновления карты
    function updatePoetMap(editor) {
        const text = editor.document.getText();
        const lines = text.split('\n');
    ...
    // Обработчик изменений в документе для подсчета слогов
    vscode.workspace.onDidChangeTextDocument(event => {
    ...
    // Команда для подсчета слогов
    let syllableCountCommand = vscode.commands.registerCommand('syllableCounter.countSyllables', () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            updateSyllableDecorations(editor);
        }
    ...
    context.subscriptions.push(syllableCountCommand, poetMapCommand);
}
function deactivate() { }
